...
class Pathviz(object):
	...
	def path_viz(self):
		...
		pl.rcParams['font.size'] = 4.0
		pl.rcParams['lines.linewidth']=0.8 
		pl.barh(range(len(enrich_path_id_up)),enrich_path_up,color = 'grey',align='center')
		ax.set_yticks(np.arange(len(enrich_path_id_up)) + width/2)
		ax.set_yticklabels(enrich_path_id_up)
		pl.title('Upregulated pathways',color='black',fontsize=8.0)
		pl.xlabel("-log10(P)")
		pl.ylabel("Pathway")
		...
		pl.rcParams['font.size'] = 4.0
		pl.rcParams['lines.linewidth']=0.8
		pl.barh(range(len(enrich_path_id_down)),enrich_path_down,color = 'grey',align='center')
		ax.set_yticks(np.arange(len(enrich_path_id_down)) + width/2)
		ax.set_yticklabels(enrich_path_id_down)
		pl.xlabel("-log10(P)")
		pl.ylabel("Pathway")
		pl.title('Downregulated pathways',color='black',fontsize=8.0)
